...
(function mobileControls(){
  const holdMap = [
    ...
    ["btnRight", "ArrowRight"],
    ["btnJet",   "Space"],     // ▲ = jump
    ["btnJump",  "KeyX"],      // Jet button => shoot
    ["btnShoot", "ArrowUp"],   // X button   => jetpack
 
    ["btnE",     "KeyE"],
  ];

  function on(el, type, fn, opt){ el.addEventListener(type, fn, opt || {passive:false}); }
  ...
  function bindHold(id, code){
    ...
    const down = (e)=>{
      // بعضی موبایل‌ها با preventDefault روی pointer مشکل پیدا می‌کنن؛
      // پس فقط برای touch حتماً preventDefault کن
    ...
    // Pointer (اگر پشتیبانی شد)
    if(window.PointerEvent){
      on(el, "pointerdown", down, {passive:false});
      on(el, "pointerup", up, {passive:false});
      on(el, "pointercancel", up, {passive:false});
      on(el, "pointerleave", up, {passive:false});
    }
    ...
    // Mouse fallback (برای تست روی دسکتاپ)
    on(el, "mousedown", down, {passive:false});
    on(el, "mouseup", up, {passive:false});
    on(el, "mouseleave", up, {passive:false});

    on(el, "contextmenu", (e)=>e.preventDefault(), {passive:false});
  }
  ...
  function bindTap(id, fn){
    ...
    if(window.PointerEvent) on(el, "pointerdown", fire, {passive:false});
    on(el, "touchstart", fire, {passive:false});
    on(el, "click", (e)=>{ e.preventDefault(); fn(); }, {passive:false});
  }
...
(function bindUIButtons(){
  ...
  // Start (Desktop)
  bindPress("startBtn", function(){
    if(state === STATE.INTRO || state === STATE.LOSE || state === STATE.WIN){
      startGame();
    }else{
      resetAll(true);
    }
  ...
  if(btn) btn.textContent = "Music: " + (audioOn ? "On" : "Off");

  if(audioOn){
    startMusic();                     // ✅ فقط این: آهنگ از startMusic میاد
